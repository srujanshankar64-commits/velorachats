...
function Room() {
  ...
  useEffect(() => {
    let active = true;
    (async () => {
      const { data } = await supabase.from("room_messages").select("id,user_id,content,created_at").eq("room_id", roomId).order("created_at", { ascending: false }).limit(50);
      if (!active || !data) return;
      const ordered = [...data].reverse() as Msg[];
      setMsgs(ordered);
    })();
  ...
  return (
    <div className="flex flex-col h-[100dvh] bg-black text-white">
      ...
      <div ref={scrollRef} className="flex-1 overflow-y-auto px-3 py-3 scrollbar-thin">
        {msgs.map((m) => (
          <div key={m.id} className={`flex mb-1.5 ${m.user_id === user?.id ? "justify-end" : "justify-start"}`}>
            <div className={`max-w-[80%] px-3.5 py-2.5 text-[15px] rounded-[18px] ${m.user_id === user?.id ? "bg-[#7C3AED] rounded-br-[4px]" : "bg-[#1C1C1E] rounded-bl-[4px]"}`}>{m.content}</div>
          </div>
        ))}
      </div>
